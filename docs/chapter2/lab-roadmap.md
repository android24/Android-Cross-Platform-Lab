...
本页把“跨平台智能座舱电子手册与车辆助手”拆成一组可以逐步完成的实验任务。它的目标是让课程从“知道要学什么”变成“知道这一周要做什么、交什么、如何复盘”。
...
- 创建 `native-android/` Kotlin + Compose 工程，并保留 `core/`、`feature/`、`benchmark/` 分层。
- 实现横屏优先的目录页和详情页，小屏可退化为单列导航。
- 接入 `shared-assets/` 中的手册和车辆数据，避免 UI 写死业务内容。
- 使用 UI State 和 Repository 管理页面状态、搜索、收藏或历史。
- 实现一个原生能力入口，例如通知、文件选择或相机。
- 记录一次基础启动或内存数据，并写明设备、构建类型和测试路径。
...
- 哪些数据层逻辑未来适合迁移到 KMP，哪些必须留在 Android 平台侧？
