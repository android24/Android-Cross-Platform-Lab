...
进入实验课之前，建议完成以下三个小挑战。它们不要求写完整代码，而是帮助你把前置课程中的原理转成工程判断。

### 挑战 1：画出一个渲染路径

选择 WebView、React Native、Flutter 或 Compose 中的一个方案，画出从业务状态变化到屏幕更新的大致路径。
...
- 能区分 UI 描述、渲染执行和平台显示之间的关系。
- 能说出这个路径中最可能产生性能问题的位置。

### 挑战 2：解释一次原生能力调用

选择 JavaScript Bridge、Native Module 或 Platform Channel 中的一种机制，解释一次“跨平台层调用 Android 原生能力”的过程。

交付物：

- 一条调用链说明。
- 一个可能失败的场景，例如权限拒绝、线程切换、参数错误或生命周期结束。
...
选择一个你准备在本课程中实现的技术栈，预测它在“跨平台智能座舱电子手册与车辆助手”项目中最可能遇到的两个风险。
...
- 风险不能只写“性能不好”或“调试困难”，需要说明具体发生在哪个场景。
- 至少一个风险要能在后续实验中通过数据验证。

## 下一步

完成本章后，进入 [课程二教学大纲](../chapter2/chapter2_1.md) 和 [实验路线图](../chapter2/lab-roadmap.md)，了解本课程的七篇内容、实验任务和最终评价维度。
